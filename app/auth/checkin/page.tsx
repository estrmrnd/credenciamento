...
export default function CheckInPage() {
  ...
  async function fazerCheckIn() {
    ...
    try {
      // Impedir check-in no mesmo dia
      if (credenciado.checkInAt) {
        const checkinDate = credenciado.checkInAt.toDate()
        ...
        if (mesmoDia) {
          setMensagem(
            `Este credenciado já fez check-in hoje às ${checkinDate.toLocaleTimeString('pt-BR')}.`
          )
          setLoading(false)
          return
        }
      ...
      <div className="space-y-4">
        ...
        {mensagem && (
          <p
            className={`mt-2 text-center ${
              mensagem.includes('Erro') || mensagem.includes('nenhum') || mensagem.includes('já fez')
                ? 'text-red-500'
                : 'text-green-600'
            }`}
          >
        ...
        {credenciado && (
          <div className="mt-4 p-4 border rounded bg-gray-100 dark:bg-zinc-800">
            ...
            {credenciado.checkInAt && (
              <p>
                <strong>Último Check-in:</strong>{' '}
                {credenciado.checkInAt.toDate().toLocaleString('pt-BR')}
              </p>
            )}
